...
Need to reproduce the exact same wave shape across reloads or between environments? Provide a **<code>data-wave-seed</code>** attribute to initialize the generator with a deterministic seed. Every wave generated by that element - including triggered regenerations - will follow the same sequence.

```html
 <!-- Seeded wave for reproducible snapshots -->
<dynamo-wave data-wave-seed="brand-home-hero"></dynamo-wave>
```

<dynamo-wave data-wave-seed="brand-home-hero" style="margin-bottom:1.25rem"></dynamo-wave>
